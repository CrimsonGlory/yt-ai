...
def generator(test_case, tname):
    def test_template(self):
        ...
        def get_tc_filename(tc):
            ...
            return ydl.prepare_filename(filter_dict(tc.get("info_dict", {}), cndn=lambda _, v: type(v) is not type))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    res_dict = ydl.extract_info(
                        test_url, force_generic_extractor=params.get("force_generic_extractor", False)
                    )
                except (DownloadError, ExtractorError) as err:
                    # Check if the exception is not a network related one
                    if not isinstance(err.exc_info[1], (TransportError, UnavailableVideoError)) or (
                        isinstance(err.exc_info[1], HTTPError) and err.exc_info[1].status == 503
                    ):
            ...
            if "playlist_count" in test_case:
                ...
                self.assertEqual(
                    num_entries, count, f"Expected exactly {count} entries in playlist {test_url}, but got {got}"
                )
            if "playlist_maxcount" in test_case:
                maxcount = test_case["playlist_maxcount"]
                self.assertLessEqual(
                    num_entries, maxcount, f"Expected at most {maxcount} entries in playlist {test_url}, but got more"
                )
            ...
            for tc_num, tc in enumerate(test_cases):
                ...
                self.assertTrue(os.path.exists(info_json_fn), f"Missing info file {info_json_fn}")
            ...
            if is_playlist and res_dict is not None and res_dict.get("entries"):
                ...
                res_tcs = [{"info_dict": e} for e in res_dict["entries"] if e]
