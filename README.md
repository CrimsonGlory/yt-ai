...
* Forked from [**yt-dlp**](https://github.com/yt-dlp/yt-dlp) and periodically merged with upstream (currently through [**yt-dlp@c7fb478d2**](https://github.com/yt-dlp/yt-dlp/commit/c7fb478d21e9e59524befbe23f7801bb267fb880))
...
* **New extractors**:
    ...
    * **AnonMP4** (`anonmp4.art` / `anonmp4.to`): Session `PLAY_SEED`/`PLAY_SIG` POST to `/video-api` for HLS; CDN fragments need a site `Origin` header.
    ...
    * **B9Good** (`b9good.org`): JWPlayer `/embed/` iframe (yundevo.guru); signed `/ajax/getSources` AES-GCM HLS. Request: [yt-dlp/yt-dlp#9245](https://github.com/yt-dlp/yt-dlp/issues/9245)
    ...
    * **CanliTV** (`canlitv.com`): JWPlayer HLS from `/player/index.php` with Referer. Request: [yt-dlp/yt-dlp#3180](https://github.com/yt-dlp/yt-dlp/issues/3180)
    ...
    * **deutschekinemathek** (`deutsche-kinemathek.de`): Schema.org VideoObject `contentUrl` HLS from Stream your Event, plus the streaming-program listing. Request: [yt-dlp/yt-dlp#5106](https://github.com/yt-dlp/yt-dlp/issues/5106)
    ...
    * **Feet9** (`feet9.com`): JSON-LD `VideoObject` `contentUrl` MP4, with Video.js `<source>` fallback; match locale-prefixed `/xx/` and `/xx-yy/` video URLs. Request: [yt-dlp/yt-dlp#1748](https://github.com/yt-dlp/yt-dlp/issues/1748)
    ...
    * **Gaystream** (`gaystream.pw`): Server-tab embed URLs; prefer **voe**, with Vidhide packed HLS fallback. Host-embed discovery is internal so Generic webpage extraction is not shadowed. Request: [yt-dlp/yt-dlp#3312](https://github.com/yt-dlp/yt-dlp/issues/3312)
    * **ge.movie** (`ge.movie`): Impersonate Cloudflare, then Playerjs playlists from the `em.filmix.stream` / `em.kinoflix.tv` iframe. A failed playlist JSON (HTTP 522 returns false) falls back to the embed page `file:` URL. Request: [yt-dlp/yt-dlp#13390](https://github.com/yt-dlp/yt-dlp/issues/13390)
    ...
    * **ICourse163** (`icourse163.org`): Signed `resourceRpcBean.getResourceTokenV2` plus `vod.study.163.com` HLS for public course intros. Request: [yt-dlp/yt-dlp#1016](https://github.com/yt-dlp/yt-dlp/issues/1016)
    ...
    * **IWitness** (`iwitness.usc.edu`): Contentful partnership `entryID` delegated to **Kaltura** (partner `27654`). Request: [yt-dlp/yt-dlp#5199](https://github.com/yt-dlp/yt-dlp/issues/5199)
    ...
    * **MGTOWTV** (`mgtow.tv`): PlayTube HTML5 `<source>` MP4s from watch and embed pages; publisher and category from `.publisher-name` / `.video-published`. Request: [yt-dlp/yt-dlp#7755](https://github.com/yt-dlp/yt-dlp/issues/7755)
    ...
    * **Novinky** (`novinky.cz`): Public `api-web.novinky.cz` document JSON and Seznam SDN MP4/HLS. Request: [yt-dlp/yt-dlp#4869](https://github.com/yt-dlp/yt-dlp/issues/4869)
    * **NowTVTR** (`nowtv.com.tr`, formerly `fox.com.tr`): Player `video` JSON and `/ajax/stream` HLS. Request: [yt-dlp/yt-dlp#1856](https://github.com/yt-dlp/yt-dlp/issues/1856)
    ...
    * **OmroepZeeland** (`omroepzeeland.nl`): BlueBillywig `regiogroei_zeeland_web_videoplayer` clip JSON for MP4/HLS. Request: [yt-dlp/yt-dlp#4676](https://github.com/yt-dlp/yt-dlp/issues/4676)
    ...
    * **OnTVTime** (`ontvtime.ru`): Player cookies (`tv`/`tv2`/`tv1`) plus page `gid` to native HLS (`s*.ontvtime.ru/stream/`). The site currently sends a GlobalSign R6 intermediate for an R46 leaf, so Python certificate verification fails. Request: [yt-dlp/yt-dlp#5172](https://github.com/yt-dlp/yt-dlp/issues/5172)
    ...
    * **Perverzija** (`tube.perverzija.com`): XtremeStream player `xs1.php` HLS from the iframe embed. Request: [yt-dlp/yt-dlp#4596](https://github.com/yt-dlp/yt-dlp/issues/4596)
    * **Piczel** (`piczel.tv`): Public `api.piczel.tv/streams/{slug}` JSON for LL-HLS live (`playback.piczel.tv`) and recording MP4s. Request: [yt-dlp/yt-dlp#16032](https://github.com/yt-dlp/yt-dlp/issues/16032)
    ...
    * **Pornzog** (`pornzog.com`): Player iframe from Txxx-family hosts (`videotxxx.com`/`vid-vx.com` aliases), delegated to **txxx**. Request: [yt-dlp/yt-dlp#2982](https://github.com/yt-dlp/yt-dlp/issues/2982)
    * **PragmaticWorksTraining** (`learning.pragmaticworkstraining.com`): Public LMS `GetCoursePublic*` JSON; course-intro Mux HLS. Lesson `GetVideoPreview` requires login. Request: [yt-dlp/yt-dlp#10188](https://github.com/yt-dlp/yt-dlp/issues/10188)
    * **PreserveTube** (`preservetube.com`): Public `/video/{id}` JSON API and HTML5 fallback; channel archives from `/channel/{id}/videos`. HTML fallback is internal so Generic webpage extraction is not shadowed. Request: [yt-dlp/yt-dlp#17540](https://github.com/yt-dlp/yt-dlp/issues/17540)
    ...
    * **SexBJCam** (`sexbjcam.com`): Impersonate Cloudflare and extract packed JWPlayer HLS. A mirror that times out is skipped; the playrecord.biz playlist is used. Request: [yt-dlp/yt-dlp#15338](https://github.com/yt-dlp/yt-dlp/issues/15338)
    ...
    * **Stuff** (`stuff.co.nz`): Impersonate plus `Accept` on `/api/v1.0/stuff/story/{id}` JSON; Brightcove player or hosted HLS/MP4. Request: [yt-dlp/yt-dlp#14961](https://github.com/yt-dlp/yt-dlp/issues/14961)
    ...
    * **ThreeNow** (`threenow.co.nz`): Live channels from the public `now-api.fullscreen.nz` `live-epg` HLS; default `age_limit` 0 when classification is missing. Request: [yt-dlp/yt-dlp#17082](https://github.com/yt-dlp/yt-dlp/issues/17082)
    ...
    * **TokinoSoraFC** (`tokinosora-fc.com`): Sheeta `/site/settings.json` plus `api.tokinosora-fc.com/fc` video_pages JSON and guest HLS session. Request: [yt-dlp/yt-dlp#4655](https://github.com/yt-dlp/yt-dlp/issues/4655)
    * **TokyoMotion** (`tokyomotion.net`): HTML5 `<source>` SD/HD `/vsrc/` MP4s from video and embed pages. Request: [yt-dlp/yt-dlp#4567](https://github.com/yt-dlp/yt-dlp/issues/4567)
    ...
    * **Udio** (`udio.com`): Public `/api/songs` JSON. Playback is `stream.udio.com/stream/{id}` HLS and is reported as DRM when the media playlist is SAMPLE-AES; GCS `song_path`/`video_path` are used only when no stream manifest is published. Request: [yt-dlp/yt-dlp#10045](https://github.com/yt-dlp/yt-dlp/issues/10045)
    ...
    * **UploadNow** (`uploadnow.io`): Firebase anonymous token, then `/api/file/search/folder-content` for `/files/` and `/f/` (and `/share?utm_source=`) folders, `/api/file/search/file` for `/s/` files, and `/api/file/downloads/links` CDN URLs
    ...
    * **WCOStream** (`wcostream.tv`): Impersonate Cloudflare, map embed `file=` to `getvidlink.php` (`cizgi`/`neptun`) tokens, and download `getvid` MP4s. Player-embed discovery is internal so Generic webpage extraction is not shadowed. Request: [yt-dlp/yt-dlp#13987](https://github.com/yt-dlp/yt-dlp/issues/13987)
...
* **Extractor fixes** (verified on live sites where possible):
    ...
    * **hungama**: Playable API with web/free device headers
    ...
    * **kika**: Restrict playlist `_VALID_URL` so `/videos/` episode URLs stay on **kika**
    ...
    * **lenta**: Rambler `getPlayerData` HLS from article `data-vid` embeds
    ...
    * **mediastream**: Scan page JSON for mdstrm.com embed and live-stream URLs, not only iframe/JSON-LD
    ...
    * **niconico**: Search pages collect `/watch/` video IDs and set `display_id`
    ...
    * **nosnl**: Read Next.js `pageProps.article` content (`VideoElement` / `AudioElement` HLS and MP3); treat nested `/video/` article URLs as videos
    ...
    * **rtl.lu**: Extract ReplayVideo HLS and ReplayAudio MP3 from current Brightspot/Next.js article pages; do not match live `/tele/live` or radio show URLs
    ...
    * **tunein**: Match program pages under `/radio/` as well as `/podcasts/`
    ...
    * **twitcasting**: Treat `data-status="live"` as live
    ...
    * **ustream**: Fetch IBM Video Streaming HLS over HTTPS UMS long-poll when `media_urls` is empty
    * **ustudio:embed**: Fetch embed `config.json` over HTTPS and read `image_url` posters
    * **vqq:video**: Fall back to the union / float_vinfo2 APIs when pinia/OG metadata is gone; empty description/series when cover JSON is missing
