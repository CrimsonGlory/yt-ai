...
class ShoutTVBaseIE(InfoExtractor):
    ...
    # version and key taken from https://watch.shout-tv.com/code/js/app.ac53bb8c011b98797a70.js
    _APP_VERSION = '6.60.0.613741b'  # 'OUTPUT_FOLDER' in JS
    ...
    def _call_api(self, content_id, content_type, note='API JSON', query=None, headers=None):
        ...
        return self._download_json(
            f'{self._API_BASE_URL}/v4/{endpoint}/{content_id}',
            content_id,
            f'Downloading {note}',
            f'Unable to download {note}',
            query=query,
            headers={
                **self._API_HEADERS,
                'Authorization': f'Bearer {ShoutTVBaseIE._ACCESS_TOKEN}',
    ...
    def _extract_vod_formats_and_subtitles(self, player, video_id):
        ...
        for idx, mpd_url in enumerate(traverse_obj(player, ('dash', ..., 'url', {url_or_none})), start=1):
            fmts, subs = self._extract_mpd_formats_and_subtitles(mpd_url, video_id, mpd_id=f'dash-{idx}', fatal=False)
            # DASH audio formats will always be sorted below HLS unless we also set 'quality'
            for q, fmt in enumerate(
                sorted(traverse_obj(fmts, (lambda _, v: v['tbr'] and v['vcodec'] == 'none')), key=lambda x: x['tbr']),
            ):
                fmt['quality'] = q
            formats.extend(fmts)
            self._merge_subtitles(subs, target=subtitles)
...
class ShoutTVIE(ShoutTVBaseIE):
    ...
    _TESTS = [
        ...
        {
            'url': 'https://watch.shout-tv.com/live/265696',
            'info_dict': {
                'id': '265696',
                'ext': 'mp4',
                'title': r're:Farscape \d{4}-\d{2}-\d{2} \d{2}:\d{2}',
                'live_status': 'is_live',
                'thumbnail': r're:https://img\.dge-prod\.dicelaboratory\.com/.+',
            },
    ...
    def _real_extract(self, url):
        ...
        details = self._call_api(
            video_id,
            video_type,
            query={'includePlaybackDetails': 'URL'},
            headers={
                'CM-APP-NAME': 'Website',
        ...
        player = self._download_json(
            details['playerUrlCallback'],
            video_id,
            'Downloading player JSON',
            'Unable to download player JSON',
...
class ShoutTVPlaylistBaseIE(ShoutTVBaseIE):
    ...
    def _fetch_page(self, playlist_id, page=1, last_seen=None):
        return self._call_api(
            playlist_id,
            self._PLAYLIST_TYPE,
            note=f'{self._PLAYLIST_TYPE} page {page}',
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(playlist_id, first_page),
            playlist_id,
            **traverse_obj(
                first_page,
                {
...
class ShoutTVSeasonIE(ShoutTVPlaylistBaseIE):
    ...
    def _create_entry(self, entry):
        return self.url_result(
            f"https://watch.shout-tv.com/video/{entry['id']}", ShoutTVIE, **self._parse_details(entry),
        )
...
class ShoutTVSeriesIE(ShoutTVPlaylistBaseIE):
    ...
    def _create_entry(self, entry):
        return self.url_result(
            f"https://watch.shout-tv.com/season/{entry['id']}",
            ShoutTVSeasonIE,
            **traverse_obj(
                entry,
                {
