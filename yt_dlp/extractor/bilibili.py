...
class BilibiliBaseIE(InfoExtractor):
        ...
        missing_formats = join_nonempty(
            *[
                ...
                for fmt in traverse_obj(
                    play_info, ("support_formats", lambda _, v: v["quality"] not in parsed_qualities)
                )
            ],
            delim=", ",
        )
    ...
    def _get_wbi_key(self, video_id):
        ...
        lookup = "".join(
            traverse_obj(
                session_data,
                ("data", "wbi_img", ("img_url", "sub_url"), {lambda x: x.rpartition("/")[2].partition(".")[0]}),
            )
        )
    ...
    def _sign_wbi(self, params, video_id):
        ...
        params = {k: "".join(filter(lambda char: char not in "!'()*", str(v))) for k, v in sorted(params.items())}
    ...
    def _download_playinfo(self, bvid, cid, headers=None, query=None, fatal=True):
        ...
        if code == 0:
            ...
            msg = join_nonempty(
                "Unable to download video info", code, traverse_obj(playurl_raw, ("message", {str})), delim=": "
            )
    ...
    def _get_subtitles(self, video_id, cid, aid=None):
        ...
        for s in traverse_obj(
            video_info, ("data", "subtitle", "subtitles", lambda _, v: v["subtitle_url"] and v["lan"])
        ):
    ...
    def _get_episodes_from_season(self, ss_id, url):
        ...
        for entry in traverse_obj(
            season_info, ("result", "main_section", "episodes", lambda _, v: url_or_none(v["share_url"]) and v["id"])
        ):
...
class BiliBiliIE(BilibiliBaseIE):
    ...
    def _real_extract(self, url):
        ...
        try:
            webpage, urlh = self._download_webpage_handle(url, video_id, headers=headers, impersonate=True)
        ...
        initial_state = self._search_json(
            r"window\.__INITIAL_STATE__\s*=", webpage, "initial state", video_id, default=None
        )
        if not initial_state:
            ...
            detail = self._download_json(
                ...
                query=self._sign_wbi(query, video_id),
                headers=headers,
            )
        ...
        if is_interactive:
            return self.playlist_result(
                self._get_interactive_entries(video_id, cid, metainfo, headers=headers),
                **metainfo,
        ...
        if not self.is_logged_in or not play_info:
            if dl_play_info := self._download_playinfo(
                video_id, cid, headers=headers, query={"try_look": 1}, fatal=False
            ):
        ...
        if video_data.get("is_upower_exclusive"):
            ...
            if "试看" in traverse_obj(play_info, ("accept_description", ..., {str})):
                self.report_warning(
                    f"This is a supporter-only video, only the preview will be extracted: {msg}", video_id=video_id
                )
        ...
        if not traverse_obj(play_info, "dash"):
            ...
            if flv_formats and len(flv_formats) < len(formats):
                ...
                if not self._configuration_arg("prefer_multi_flv"):
                    dropped_fmts = ", ".join(f"{f.get('format_note')} ({f.get('format_id')})" for f in flv_formats)
                    ...
                    formats = traverse_obj(
                        ...
                        formats,
                        lambda _, v: v["quality"] == int(self._configuration_arg("prefer_multi_flv")[0]),
...
class BiliBiliBangumiIE(BilibiliBaseIE):
    ...
    def _real_extract(self, url):
        ...
        headers = self.geo_verification_headers()
        try:
            webpage = self._download_webpage(url, episode_id, headers=headers, impersonate=True) or ""
        except ExtractorError as e:
            if not (isinstance(e.cause, HTTPError) and e.cause.status in (403, 412)):
                raise
            webpage = ""
        ...
        play_info = self._search_json(
            r"playurlSSRData\s*=", webpage, "embedded page info", episode_id, end_pattern="\n", default=None
        ...
        geo_blocked = traverse_obj(
            play_info, ("plugins", lambda _, v: v["name"] == "AreaLimitPanel", "config", "is_block", {bool}, any)
        )
        ...
        if traverse_obj(
            ...
            (
                ...
                any,
                {lambda x: x in ("PLAY_PREVIEW", "preview")},
            ),
        ):
            self.report_warning(
        ...
        episode_number, episode_info = next(
            (
                ...
                for idx, ep in enumerate(
                    traverse_obj(bangumi_info, (("episodes", ("section", ..., "episodes")), ..., {dict})), 1
                )
        ...
        season_number, season_title = season_id and next(
            (
                ...
                for idx, e in enumerate(traverse_obj(bangumi_info, ("seasons", ...)))
...
class BilibiliCheeseBaseIE(BilibiliBaseIE):
    def _extract_episode(self, season_info, ep_id):
        episode_info = traverse_obj(season_info, ("episodes", lambda _, v: v["id"] == int(ep_id)), get_all=False)
...
class BilibiliSpaceVideoIE(BilibiliSpaceBaseIE):
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(page_idx):
            ...
            if status_code == -401:
                raise ExtractorError("Request is blocked by server (401), please wait and try later.", expected=True)
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(page_idx):
            return self._download_json(
                ...
                query={"uid": playlist_id, "pn": page_idx + 1, "ps": 30, "order": 1, "jsonp": "jsonp"},
            )["data"]
        ...
        def get_entries(page_data):
            ...
            for entry in page_data.get("data") or []:
                yield self.url_result(
                    f"https://www.bilibili.com/audio/au{entry['id']}", BilibiliAudioIE, str_or_none(entry["id"])
                )
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(page_idx):
            return self._download_json(
                ...
                note=f"Downloading page {page_idx}",
                headers={"Referer": url},
                query={"mid": mid, "season_id": sid, "page_num": page_idx + 1, "page_size": 30},
            )["data"]
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(page_idx):
            return self._download_json(
                ...
                note=f"Downloading page {page_idx}",
                headers={"Referer": url},
                query={"mid": mid, "series_id": sid, "pn": page_idx + 1, "ps": 30},
            )["data"]
...
class BilibiliFavoritesListIE(BilibiliSpaceListBaseIE):
    _VALID_URL = (
        r"https?://(?:space\.bilibili\.com/\d+/favlist/?\?fid=|(?:www\.)?bilibili\.com/medialist/detail/ml)(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        entries = self._get_entries(
            self._download_json(
                f"https://api.bilibili.com/x/v3/fav/resource/ids?media_id={fid}", fid, note="Download favlist entries"
            ),
            "data",
        )
...
class BilibiliWatchlaterIE(BilibiliSpaceListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        watchlater_info = self._download_json("https://api.bilibili.com/x/v2/history/toview/web?jsonp=jsonp", list_id)
...
class BilibiliCategoryIE(InfoExtractor):
    ...
    def _fetch_page(self, api_url, num_pages, query, page_num):
        ...
        for video in video_list:
            yield self.url_result("https://www.bilibili.com/video/{}".format(video["bvid"]), "BiliBili", video["bvid"])
    ...
    def _entries(self, category, subcategory, query):
        ...
        return OnDemandPagedList(functools.partial(self._fetch_page, api_url, num_pages, query), size)
...
class BilibiliAudioBaseIE(InfoExtractor):
    def _call_api(self, path, sid, query=None):
        if not query:
            query = {"sid": sid}
        return self._download_json("https://www.bilibili.com/audio/music-service-c/web/" + path, sid, query=query)[
            "data"
        ]
...
class BilibiliAudioAlbumIE(BilibiliAudioBaseIE):
    ...
    def _real_extract(self, url):
        ...
        songs = self._call_api("song/of-menu", am_id, {"sid": am_id, "pn": 1, "ps": 100})["data"]
        ...
        for song in songs:
            ...
            if not sid:
                continue
            entries.append(self.url_result("https://www.bilibili.com/audio/au" + sid, BilibiliAudioIE.ie_key(), sid))
        ...
        if entries:
            ...
            if album_title:
                ...
                return self.playlist_result(entries, am_id, album_title, album_data.get("intro"))
...
class BiliBiliPlayerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(f"http://www.bilibili.tv/video/av{video_id}/", ie=BiliBiliIE.ie_key(), video_id=video_id)
...
class BiliBiliDynamicIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        # Chrome 145+ UAs are risk-controlled (code -352) on this endpoint.
...
class BiliIntlBaseIE(InfoExtractor):
    ...
    def json2srt(self, json):
        return "\n\n".join(
            ...
            for i, line in enumerate(traverse_obj(json, ("body", lambda _, l: l["content"] and l["from"] and l["to"])))
        )
...
class BiliIntlIE(BiliIntlBaseIE):
    ...
    def _extract_video_metadata(self, url, video_id, season_id):
        ...
        initial_data = self._search_json(
            r"window\.__INITIAL_(?:DATA|STATE)__\s*=", webpage, "preload state", video_id, default={}
    ...
    def _get_comments_reply(self, root_id, next_id=0, display_id=None):
        ...
        if not traverse_obj(comment_api_raw_data, ("data", "cursor", "is_end")):
            yield from self._get_comments_reply(root_id, comment_api_raw_data["data"]["cursor"]["next"], display_id)
    ...
    def _real_extract(self, url):
        ...
        if ep_id:
            intro_ending_json = (
                self._call_api(f"/web/v2/ogv/play/episode?episode_id={ep_id}&platform=web", video_id, fatal=False) or {}
            )
                ...
                chapters = [
                    {
                        "start_time": float_or_none(
                            traverse_obj(intro_ending_json, ("skip", "opening_start_time")), 1000
                        ),
                    ...
                    {
                        "start_time": float_or_none(
                            traverse_obj(intro_ending_json, ("skip", "ending_start_time")), 1000
                        ),
...
class BiliIntlSeriesIE(BiliIntlBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bili(?:bili\.tv|intl\.com)/(?:[a-zA-Z]{2}/)?(?:play|media)/(?P<id>\d+)/?(?:[?#]|$)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        series_info = (
            self._call_api(f"/web/v2/ogv/play/season_info?season_id={series_id}&platform=web", series_id).get("season")
            or {}
        )
        return self.playlist_result(
