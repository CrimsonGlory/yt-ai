...
class BloggerIE(InfoExtractor):
    ...
    def _extract_rpc_data(self, webpage, token_id):
        wiz_global_data = self._search_json(
            r'window\.WIZ_global_data\s*=', webpage, 'wiz global data', token_id)
        ...
        player_data = self._download_webpage(
            ...
            token_id, 'Downloading player data', headers={
        ...
        for line in player_data.splitlines():
            ...
            parsed_line = self._parse_json(line, token_id, fatal=False)
            if payload := traverse_obj(parsed_line, (
                lambda _, v: v[0] == 'wrb.fr' and v[1] == self._RPC_ID,
                2, {str}, any,
            )):
                rpc_data = self._parse_json(payload, token_id, fatal=False)
                if isinstance(rpc_data, list):
                    return rpc_data

        raise ExtractorError('Unable to extract player data')
    ...
    def _real_extract(self, url):
        ...
        return {
            'formats': formats,
            'duration': traverse_obj(formats, (..., 'duration', {float_or_none}, any)),
            **traverse_obj(rpc_data, {
                'id': (5, {str_or_none}, {require('video id')}),
