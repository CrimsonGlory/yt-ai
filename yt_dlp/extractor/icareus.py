...
class IcareusIE(InfoExtractor):
    ...
    _VALID_URL = [
        rf'(?P<base_url>https?://(?:www\.)?(?:{_DOMAINS}))/[^?#]+/player/[^?#]+\?(?:[^#]+&)?(?:assetId|eventId)=(?P<id>\d+)',
        rf'(?P<base_url>https?://(?:www\.)?(?:{_DOMAINS}))/(?:[a-z]{{2}}(?:_[A-Z]{{2}})?/)?(?:video|event)/details/(?P<id>\d+)',
    ]
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'thumbnail': 'https://dvcf59enpgt5y.cloudfront.net/image/image_gallery?img_id=68022501',
            'duration': 293,
            'upload_date': '20200924',
            'timestamp': 1600938312,
        },
    }, {  # Recorded livestream
        ...
        'url': 'https://www.helsinkikanava.fi/en/event/details/76241489',
        'only_matching': True,
    }, {
    ...
    def _extract_playback(self, assets, video_id):
        subtitles = {}
        for item in assets.get('subtitles') or []:
            if len(item) < 3:
                continue
            surl = url_or_none(item[2])
            if not surl:
                continue
            lang = remove_end((item[1] or '').split(' ')[0], ':') or 'und'
            subtitles.setdefault(lang, []).append({'url': surl})
    ...
    def _extract_nextjs(self, webpage, temp_id):
        nextjs = self._search_nextjs_v13_data(webpage, temp_id)
        player = traverse_obj(nextjs, (
            lambda _, v: isinstance(v, dict) and isinstance(v.get('videoInfo'), dict),
            any, {require('player data')}))
        asset = player.get('asset') or {}
        event = traverse_obj(nextjs, (
            lambda _, v: isinstance(v, dict) and v.get('eventId') and v.get('startTime'),
            any)) or {}

        return (
            str_or_none(player.get('assetId') or player.get('id')) or temp_id,
            player['videoInfo'],
            {
                'title': asset.get('name') or self._og_search_title(webpage, default=None),
                'description': asset.get('description') or self._og_search_description(webpage),
                'timestamp': int_or_none(
                    asset.get('created') or asset.get('date') or event.get('startTime'), scale=1000),
                'duration': int_or_none(asset.get('duration')),
                'thumbnail': url_or_none(
                    asset.get('thumbnailMedium') or player['videoInfo'].get('thumbnail')),
            },
        )

    def _real_extract(self, url):
        base_url, temp_id = self._match_valid_url(url).group('base_url', 'id')
        webpage = self._download_webpage(url, temp_id)

        video_id = self._search_regex(
            r"_icareus\['itemId'\]\s*=\s*'(\d+)'", webpage, 'video_id', default=None)
        if video_id:
            organization_id = self._search_regex(
                r"_icareus\['organizationId'\]\s*=\s*'(\d+)'", webpage, 'organization_id')
            assets = self._download_json(
                ...
                video_id, data=urlencode_postdata({
                    ...
                    'token': self._search_regex(
                        r"_icareus\['token'\]\s*=\s*'([a-f0-9]+)'", webpage, 'icareus_token'),
                }))
            ...
            video_id, assets, info = self._extract_nextjs(webpage, temp_id)

        formats, subtitles = self._extract_playback(assets, video_id)
