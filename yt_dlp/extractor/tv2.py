...
class TV2IE(InfoExtractor):
    _VALID_URL = r'https?://(?:www\.)?tv2\.no/v(?:ideo)?\d*/(?:[^?#]+/)*(?:[A-Za-z0-9]+-)?(?P<id>\d+)'
    _TESTS = [{
        'url': 'https://www.tv2.no/video/nyheter/her-kolliderer-romsonden-med-asteroiden/20083301',
        'md5': 'f26dbcee2117d45b95fb21804236f3a0',
        'info_dict': {
            'id': '20083301',
            'ext': 'mp4',
            ...
            'categories': ['asteroide', 'nasa', 'romfart', 'romsonde', 'verdensrommet'],
        },
    }, {
        ...
        'url': 'https://www.tv2.no/video/nyheter/oyeblikkene-vi-husker/cpMJHQi6bl-20356314',
        'only_matching': True,
    }, {
        'url': 'https://www.tv2.no/video/asset/cpMJHQi6bl-20356314',
        'only_matching': True,
    }]
    _GEO_COUNTRIES = ['NO']
    _DISCOVERY_API = 'https://tv2news.content-discovery.cf.eu-north-1-prod.vmnd.tv'
    _PLAY_API = 'https://tv2news.play.cf.eu-north-1-prod.vmnd.tv'

    def _real_extract(self, url):
        video_id = self._match_id(url)
        asset = traverse_obj(self._download_json(
            f'{self._DISCOVERY_API}/api/v1/assets', video_id,
            'Downloading metadata JSON', query={
                'platform': 'web',
                'query': f'id:{video_id} OR legacyAssetId:{video_id}',
            }), ('data', 0))
        if not asset:
            raise ExtractorError('Unable to find video', expected=True)

        asset_id = asset['id']
        is_live = asset.get('isLive') is True

        try:
            playback = self._download_json(
                Request(
                    f'{self._PLAY_API}/api/v2/asset/{asset_id}/play',
                    method='POST',
                    headers={
                        'Origin': 'https://www.tv2.no',
                        'Referer': 'https://www.tv2.no/',
                    },
                    query={'contentType': 'application/x-mpegurl'},
                ),
                asset_id, 'Downloading playback JSON')
        ...
        play = traverse_obj(playback, ('data', 0), default={})
        if play.get('isDrmProtected') or asset.get('drmProtected'):
            self.report_drm(asset_id)

        stream_url = traverse_obj(
            play, ('recommendedStream', 'manifest', 'uri', {url_or_none}))
        if not stream_url:
            raise ExtractorError('No playable streams found', expected=True)

        formats = self._extract_m3u8_formats(
            stream_url, asset_id, 'mp4', m3u8_id='hls', live=is_live)

        keywords = asset.get('keywords')
        return {
            'id': asset_id,
            'title': asset.get('title') or play.get('assetTitle'),
            'description': strip_or_none(asset.get('description')),
            'thumbnail': traverse_obj(asset, ('images', 'defaultUrl', {url_or_none})),
            'timestamp': parse_iso8601(
                asset.get('event-occurrence-time')
                or asset.get('createTime')
                or asset.get('transmissionTime')),
            'duration': float_or_none(asset.get('duration') or play.get('duration')),
            'categories': [c.strip() for c in keywords.split(',')] if keywords else None,
            'formats': formats,
            'is_live': is_live,
        }
