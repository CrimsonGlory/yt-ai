...
class GameJoltBaseIE(InfoExtractor):
    ...
    def _get_comments(self, post_num_id, post_hash_id):
        ...
        for page in itertools.count(1):
            comments_data = self._call_api(
                ...
                % (post_num_id, sort_by, "scroll_id" if is_scrolled else "page", scroll_id if is_scrolled else page),
                post_hash_id,
            ...
            for comment in traverse_obj(comments_data, (("comments", "childComments"), ...), expected_type=dict):
                yield {
                    "id": comment["id"],
                    "text": self._parse_content_as_text(self._parse_json(comment["comment_content"], post_hash_id)),
                    "timestamp": int_or_none(comment.get("posted_on"), scale=1000),
                    "like_count": comment.get("votes"),
                    "author": traverse_obj(
                        comment, ("user", ("display_name", "name")), expected_type=str_or_none, get_all=False
                    ),
    ...
    def _parse_post(self, post_data):
        ...
        description, full_description = (
            ...
            or self._parse_content_as_text(self._parse_json(post_data.get("lead_content"), post_id)),
            None,
        )
        ...
        info_dict = {
            ...
            "categories": [
                try_get(
                    category,
                    lambda x: "{} - {}".format(
                        x["community"]["name"], x["channel"].get("display_title") or x["channel"]["title"]
                    ),
                )
            ...
            gif_entries.append(
                {
                    ...
                    "formats": [
                        ...
                        for url_key in ("img_url", "mediaserver_url", "mediaserver_url_mp4", "mediaserver_url_webm")
...
class GameJoltPostListBaseIE(GameJoltBaseIE):
    def _entries(
        self, endpoint, list_id, note="Downloading post list", errnote="Unable to download post list", initial_items=[]
    ):
        page_num, scroll_id = 1, None
        items = initial_items or self._call_api(endpoint, list_id, note=note, errnote=errnote)["items"]
        while items:
            for item in items:
                post = traverse_obj(item, ("action_resource_model", {dict}))
                if post:
...
class GameJoltUserIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_data = self._call_api(
            f"web/profile/@{user_id}", user_id, note="Downloading user info", errnote="Unable to download user info"
        )["user"]
...
class GameJoltGameIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(
                f"web/posts/fetch/game/{game_id}", game_id, "Downloading game posts", "Unable to download game posts"
            ),
            game_id,
            game_data.get("title"),
            description,
...
class GameJoltCommunityIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        display_id, community_id, channel_id, sort_by = self._match_valid_url(url).group(
            "id", "community", "channel", "sort"
        )
